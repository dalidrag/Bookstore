...
class Carousel extends Component {
  ...
  getCarouselPages = () => {
    ...
    const slideTrayWidth = slideNodeRectList.reduce(
      (slideTrayWidth, slideNodeRect) => slideTrayWidth + slideNodeRect.width,
      0
    );

    return slideNodeRectList.reduce((carouselPages, slideNodeRect, index) => {
      if (carouselPages.length === 0)
        carouselPages.push({ id: 0, x: 0, slidesWidth: 0, firstIndex: 0 });
